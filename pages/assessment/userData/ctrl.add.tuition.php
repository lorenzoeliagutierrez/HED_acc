...
if (isset($_POST['submit'])) {
    ...
    foreach ($year_array as $year_value) {
        ...
        if (isset($_POST['tuition'.$i])) {

        $tuition = mysqli_real_escape_string($db, $_POST['tuition'.$i]);
        $tuition_check = mysqli_query($acc, "SELECT * FROM tbl_tuition_fees WHERE course_id = '$course_id' AND ay_id = '$ay_id' AND year_id = '$year_value'") or die(mysqli_error($acc));
